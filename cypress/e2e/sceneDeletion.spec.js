...
context("Scene Deletion", () => {
  ...
  it("Delete First", () => {
    ...
    cy.get(`[data-testid=${firstSceneId}]`)
      .contains(`${DeleteButtonText}`)
      .click({ force: true });
    cy.get(`[data-testid=${firstSceneId}]`).should("not.exist");
    const newCurrentSceneId = getSceneId(scenes[1]);
    cy.get(`[data-testid=${newCurrentSceneId}]`).should(
      "have.css",
      "border",
      SelectedSceneBorder
    );
  ...
  it("Delete Last", () => {
    ...
    cy.get(`[data-testid=${sceneIdToDelete}]`)
      .contains(`${DeleteButtonText}`)
      .click({ force: true });
    cy.get(`[data-testid=${sceneIdToDelete}]`).should("not.exist");
    cy.get(`[data-testid=${currentSceneId}]`).should(
      "have.css",
      "border",
      SelectedSceneBorder
    );
